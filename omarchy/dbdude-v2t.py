...
# v6 - Omarchy (Arch Linux + Hyprland) version - uses ydotool for text output (Wayland-safe), no sudo needed
# (hotkeys read from /dev/input via keystate.py; device access via a logind uaccess udev rule)
...
try:
    ...
    print("ERROR: ydotool not found. Install with: sudo pacman -S ydotool", file=sys.stderr)
    sys.exit(1)
...
class DbdudeV2tApp:
    ...
    def _show_help(self):
        """Show general help."""
        help_text = """dbdude-v2t for Omarchy
...
The icon appears in your top panel."""
    ...
    def run(self):
        ...
        print(">> Runs as your user (hotkeys via /dev/input; access granted by the uaccess udev rule)")
