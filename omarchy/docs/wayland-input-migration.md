...
> **Status: APPLIED in this `omarchy/` folder (2026-09-19), cloned from
> `ubuntu-26.04/`.** Omarchy (Arch Linux + Hyprland) is Wayland-only, so the
> same ydotool design applies; setup.bash installs ydotool via pacman and
> re-homes Arch's shipped ydotool.service user unit to the graphical session.
> The original `ubuntu/` folder (24.04, X11) is unchanged.
